...
const agent = process.env.HTTPS_PROXY || process.env.https_proxy || process.env.HTTP_PROXY || process.env.http_proxy
  ? new HttpsProxyAgent(process.env.HTTPS_PROXY || process.env.https_proxy || process.env.HTTP_PROXY || process.env.http_proxy)
  : null;
const os = require("os");
...
async function install() {
  ...
  if (!fs.existsSync(binaryPath)) {
    ...
    console.log(`${PACKAGE_NAME} is already installed at ${binaryPath}`);
  }
}
